...
- (void)drawTextInRect:(CGRect)rect
{
    ...
    if ([self textAlignment] == NSTextAlignmentRight) {
        ...
        if (self.strikeLabDuan)
        {
            lineRect = CGRectMake(0, rect.size.height / 2, strikeWidth-10, 1);
        }
        else
        {
            if (self.strikedistion>0)
            {
                lineRect = CGRectMake(0, rect.size.height / 2, strikeWidth+self.strikedistion, 1);
